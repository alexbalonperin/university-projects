...
% GUI_PROJECT_DR Interface graphique pour l'affichage des débits reçus.
% GUI_PROJECT_DR(VEC,DATARATE,TX) affiche l'environnement VEC, l'émetteur TX
%       et l'ensemble des débits reçus DATARATE sur toutes les zones de 
%	  l'environnement.
%
% VEC = matrice [m x 4] représentant l'environnement (murs) où m est le 
%       nombre de murs.
% DATARATE = matrice [s x t] représentant l'ensemble des débits reçus 
...
%		DATARATE représente donc le débit moyen reçu pour une
%		zone de 1 m².
...
vertices = [TX(i,1)-1/4 TX(i,2)-1/4 1/2;
    TX(i,1)-1/4 TX(i,2)+1/4 1/2;
    TX(i,1)+1/4 TX(i,2)-1/4 1/2;
    TX(i,1)+1/4 TX(i,2)+1/4 1/2;
    TX(i,1)-1/4 TX(i,2)-1/4 param.h;
    TX(i,1)-1/4 TX(i,2)+1/4 param.h;
    TX(i,1)+1/4 TX(i,2)-1/4 param.h;
    TX(i,1)+1/4 TX(i,2)+1/4 param.h];
faces = [1 2 6 5;
